...
The Networking architecture uses a Virtual Private Cloud on AWS with public and private subnets that supports Dual Stack (IPv4 and IPv6). Version 1 of the Networking is to validate the routing behaviour between each subnet and understand any failures. Are packets moving from source to destination as designed? Are VPC components configured correctly? Are the NAT Gateway, Egress-only gateway and IGW behaving as expected?
